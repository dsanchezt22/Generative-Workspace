  ...
  return (
    <div className="absolute inset-0 grid place-items-center pointer-events-none px-6">
      <div className="pointer-events-auto flex flex-col items-center gap-4 -mt-20 text-center max-w-md">
        <div className="w-12 h-12 rounded-lg bg-[var(--surface-elevated)] border border-[var(--border)] grid place-items-center text-[var(--accent)] animate-pop">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {CHIPS.map(([label, prompt]) => (
            <button
              ...
              className="rounded-sm border border-[var(--border)] bg-[var(--surface)] px-3 py-1.5 text-xs hover:border-[var(--accent)] hover:text-[var(--accent)] transition"
            >
              {label}
            </button>
          ))}
